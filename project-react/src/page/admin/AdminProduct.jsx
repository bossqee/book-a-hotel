...
import {
  ...
  BarChart3,
  BedDouble,
  History, // เพิ่ม Icon สำหรับสต็อก
} from "lucide-react";
...
const AdminProduct = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!formData.tag)
      return Swal.fire("แจ้งเตือน", "กรุณาเลือกประเภทที่พัก", "warning");
    ...
    if (editingId) {
      updatedProducts = products.map((p) =>
        p.id === editingId ? { ...finalData, id: editingId } : p,
      );
      Swal.fire({
        icon: "success",
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            ...
            <p className="text-slate-500 font-medium">
              เพิ่ม แก้ไข และดูแลสต็อกที่พักทั้งหมด
            </p>
          </div>
          <button
          ...
          <AnimatePresence>
            {filteredList.map((item) => (
              ...
              >
                <img
                  src={item.image}
                  className="w-40 h-32 object-cover rounded-2xl bg-slate-50 shadow-inner"
                  alt=""
                />
                ...
                {/* --- ส่วนแสดง Stock ใน UI หลัก --- */}
                <div className="px-6 py-2 bg-slate-50 rounded-2xl min-w-[140px] text-center border border-slate-100">
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1 flex items-center justify-center gap-1">
                    <BedDouble size={12} /> สต็อกห้องพัก
                  </p>
                  <div className="flex items-baseline justify-center gap-1">
                    <span
                      className={`text-xl font-black ${item.availableRooms > 0 ? "text-blue-600" : "text-rose-600"}`}
                    >
                      {item.availableRooms}
                    </span>
                  ...
                  <div className="w-full bg-slate-200 h-1.5 rounded-full mt-2 overflow-hidden">
                    <div
                      className={`h-full transition-all ${item.availableRooms > 3 ? "bg-blue-500" : "bg-rose-500"}`}
                      style={{
                        width: `${(item.availableRooms / item.totalRooms) * 100}%`,
                      }}
                    />
                  </div>
                </div>
                ...
                <div className="text-center md:text-right px-6 border-x border-slate-50">
                  ...
                  <p className="text-slate-300 text-sm line-through font-bold">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="p-4 bg-slate-50 text-slate-600 rounded-2xl hover:bg-rose-500 hover:text-white transition-all shadow-sm"
                  >
                    <Trash2 size={20} />
                  </button>
                </div>
              ...
              <div className="hidden md:block w-5/12 bg-slate-50 relative border-r">
                {formData.image ? (
                  ...
                  <div className="h-full flex flex-col items-center justify-center p-10 text-slate-300">
                    ...
                    <p className="text-sm font-bold uppercase mt-4 tracking-widest text-center">
                      ตัวอย่างรูปภาพ
                    </p>
                  </div>
                )}
              ...
              <div className="flex-1 flex flex-col">
                <div className="p-8 border-b flex justify-between items-center bg-white sticky top-0 z-10">
                  <div>
                    <h2 className="text-2xl font-black text-slate-900">
                      {editingId ? "แก้ไขข้อมูล" : "เพิ่มที่พักใหม่"}
                    </h2>
                    <p className="text-blue-600 text-xs font-black uppercase tracking-widest mt-1">
                      รายละเอียดพร้อมจัดการสต็อก
                    </p>
                  </div>
                  <button
                    onClick={resetForm}
                    className="p-2 hover:bg-rose-50 hover:text-rose-500 rounded-full transition-colors"
                  >
                    <X />
                  </button>
                </div>
                ...
                >
                  ...
                  <div className="space-y-4">
                    <label className="flex items-center gap-2 text-xs font-black text-slate-400 uppercase tracking-widest border-l-4 border-blue-500 pl-2">
                      ข้อมูลพื้นฐาน
                    </label>
                    <input
                      required
                  ...
                  {/* --- Section ใหม่: Stock Management --- */}
                  <div className="space-y-4">
                    <label className="flex items-center gap-2 text-xs font-black text-slate-400 uppercase tracking-widest border-l-4 border-indigo-500 pl-2">
                      จัดการสต็อกห้องพัก
                    </label>
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="relative">
                        <History
                          className="absolute left-4 top-5 text-indigo-500"
                          size={18}
                        />
                        <input
                          required
                          type="number"
                          placeholder="จำนวนห้องที่เหลือ"
                          className="modern-input !pl-12"
                  ...
                  {/* Section 3: Pricing & Stats */}
                  <div className="space-y-4">
                    <label className="flex items-center gap-2 text-xs font-black text-slate-400 uppercase tracking-widest border-l-4 border-amber-500 pl-2">
                      ราคาและคะแนน
                    </label>
                    <div className="grid grid-cols-2 gap-4">
                      <input
                      ...
                      <input
                        ...
                        className="modern-input opacity-70"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <input
                        type="number"
                        placeholder="รีวิว (เว้นเพื่อสุ่ม)"
                        className="modern-input"
                  ...
                  {/* Section 4: Media */}
                  <div className="space-y-4">
                    <label className="flex items-center gap-2 text-xs font-black text-slate-400 uppercase tracking-widest border-l-4 border-emerald-500 pl-2">
                      รูปภาพและรายละเอียด
                    </label>
                    <input
                      required
                      ...
                      className="modern-input font-mono text-[11px]"
                  ...
                  <button
                    type="submit"
                    className="w-full bg-slate-900 text-white p-5 rounded-3xl font-black text-lg flex items-center justify-center gap-3 hover:bg-blue-600 transition-all shadow-xl active:scale-95"
                  >
                    <Save size={22} />{" "}
                    {editingId ? "บันทึกการแก้ไข" : "ยืนยันเพิ่มที่พักใหม่"}
                  </button>
                </form>
              </div>
            </motion.div>
